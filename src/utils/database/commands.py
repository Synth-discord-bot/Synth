...
class CommandDatabase(BaseDatabase):
    ...
    async def set_cooldown(self, guild_id: int, command: str, cooldown: int):
        ...
        if commands:
            for command_ in commands:
                ...
                commands.append(
                    {"name": command, "cooldown": cooldown, "disabled": False}
                )
        else:
    ...
    async def _get_commands_list(self, guild_id: int) -> List[Dict[str, Any]]:
        ...
        return (await self.find_one_from_db({"id": guild_id})).get("commands", [])
    ...
    async def check_command(
        ...
        # check if a command list exists from a database
        else:
            if (
                not (_ := await self.find_one_from_db({"id": guild_id}))
                and add_if_not_exists
            ):
                # try to find command in a database
                commands = (await self.find_one_from_db({"id": guild_id})).get(
                    "commands", []
                )
                if commands:
    ...
    async def add_command(self, guild_id: int, command: str) -> None:
        if await self.find_one_from_db({"id": guild_id}) is None:
            ...
            return await self.update_db({"id": guild_id}, {"commands": [command]})
