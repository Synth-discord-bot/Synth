...
class ButtonOrSelect(disnake.ui.Select):
    ...
    async def callback(self, interaction: disnake.MessageInteraction) -> None:
        ...
        #description
        await interaction.channel.send(f"New title: `{name}`.\n\nWrite new description:")
        msg = await self.bot.wait_for(
        ...
        #channel
        await interaction.channel.send(f"New title: `{name}`\n\nNew description: `{msg.content}`. Write id or mention of channel:", delete_after = 30)
        msg = await self.bot.wait_for(
            "message", check=lambda x: x.author == interaction.author, timeout=None
        )
        ...
        try:
            channel = await TextChannelConverter().convert(ctx=interaction, argument=channel_id)
        except BadArgument:
...
class FormsView(disnake.ui.View):
    ...
    async def create_form(self, _: disnake.ui.Button, interaction: disnake.Interaction, name: str) -> None:
        FormEmbed = disnake.Embed(colour=0x43adf3)
        FormEmbed.title = "Creating Form"
        FormEmbed.description = "To create a form, please choose one of the options below:"
    ...
    async def edit_form(self, _: disnake.ui.Button, interaction: disnake.Interaction) -> None:
        await interaction.send("Form has been edited", ephemeral=True, view=FormsView(self.bot))

    @disnake.ui.button(label="Delete", style=disnake.ButtonStyle.red)
    async def delete_form(self, _: disnake.ui.Button, interaction: disnake.Interaction) -> None:
        await interaction.send("Form has been deleted", ephemeral=True, view=FormsView(self.bot))
...
class EditFormView(disnake.ui.View):
    ...
    @disnake.ui.button(label = "Title", custom_id="title_button", style = disnake.ButtonStyle.gray)
...
class FormsCog(commands.Cog):
    ...
    @form.sub_command()
    async def create(self, interaction: disnake.MessageCommandInteraction, name: str) -> None:
        setup_form_embed = disnake.Embed(title = "Title", description= "Description")
        button_or_select = disnake.ui.Button(label = "Button/Select", style=disnake.ButtonStyle.gray, disabled=True)

        select = disnake.ui.View()
        select.add_item(ButtonOrSelect(self.bot, forms=self.forms))

        await interaction.send(f"Create a {name} form. Please choose one of the options below. Setup form:", view=select)
        await interaction.channel.send(embed=setup_form_embed, components=[button_or_select])
    ...
    @form.sub_command()
    async def delete(self, interaction: disnake.MessageCommandInteraction, name: str) -> None:
        await interaction.send(f"Nigga `{name}`", ephemeral=True, view = FormsView(self.bot))
