            ...
            try:
                ...
                for text_channel in ctx.guild.text_channels:
                    text_channel_data = {
                        ...
                        "topic": text_channel.topic.replace(".", "") if text_channel.topic else None,
                ...
                if backups.check_backup(ctx.guild):
                    await backups.add_to_db({"id": ctx.guild.id, "backup_data": backup_data})
                else:
                    await backups.add_to_db({"id": ctx.guild.id, "backup_data": backup_data})
            ...
            for k in range(249, -1, -1):
                try:
                    await ctx.guild.create_role(
                        ...
                        permissions=disnake.Permissions(permissions=data['roles'][str(k)]['perms']),
                    ...
                    for role in ctx.guild.roles:
                        try:
                            ovw = disnake.PermissionOverwrite.from_pair(
                                disnake.Permissions(permissions=raw_overwrites[role.name]['a']),
                                disnake.Permissions(permissions=raw_overwrites[role.name]['d'])
                            )
                            overwrites[role] = ovw
                        except (Exception, ExceptionGroup):
                            pass
